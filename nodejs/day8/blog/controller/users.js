...
const register=(req, res) => {
    ...
    conn.query(sql1, body.username, (err, result) => {
        ...
        //在执行sql语句之前，先对用户提供的密码，做一层加密，防止密码被泄露之后，明文被盗取清空
        // 加密的方法
...
const login=(req,res)=>{ 
    ...
    //     //把登录成功之后的信息挂载到sessiob上面， 把用户登录成功之后的结果挂载到session上
    ...
    const sql2='select *from blog_users where username=?'
    conn.query(sql2,[body.username],(err,result)=>{
        ...
        // console.log(result)
        // console.log(req.session)
        // bcrypt.compare('用户输入密码','数据库中记录的密码',回调函数)
        // console.log(result[0].password)
        // console.log(body.password)
        bcrypt.compare(body.password,result[0].password,(err,comres)=>{
            // console.log(err)
            if(err) return res.send({msg:'用户登录失败',static:503})

            if(!comres) return res.send({msg:'用户登录失败504',static:504})
...
const logout=(req,res)=>{
    //把session中存储的数据也要初始化
    req.session.destroy(()=>{
        //使用res.redirect 方法，可以让客户端 重新访问指定的页面 这里是访问首页就可以了
        res.redirect('/')
    })
